            ...
            if (isMultipart) {
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("file_photo1"))//write the name of image here it is img_product
                        {
                            ...
                            photo = "WorkerPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("file_proof1"))//write the name of image here it is img_product
                        {
                            String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            ...
                            proof = "WorkerProof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
                ...
                if (value[7].equals(value[8])) {//Strinh str1="insert into tbl_shop ()";//inserting file
                    String InsQry = "INSERT INTO `tbl_worker`( `worker_name`, `worker_contact`, `worker_email`,`worker_address`,place_id, `worker_photo`,`worker_proof`,`worker_password`)"
